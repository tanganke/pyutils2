...
class RealNVP(nn.Module):
    ...
    def forward(self, x, reverse=False) -> torch.Tensor:
        """_summary_
        ...
        Returns:
            torch.Tensor: 
                if reverse is False: log(p(x)), sldj
        """
        sldj = None
        if not reverse:
        ...
        return x, sldj #? What are the outputs?
    ...
    @property
    def is_cuda(self):
        return next(iter(self.parameters())).is_cuda
    ...
    def loss(self, x):
        ...
        z, sldj = self(x, reverse=False)
        loss = loss_fn(z, sldj)

    def fit(self, trainloader, optimizer=None, lr=1e-4, weight_decay=1e-2, num_epoch=1, max_grad_norm: int = None):
        loss_fn = RealNVPLoss()
        ...
        for epoch_id in pbar:
            for batch_id, (x, _) in enumerate(trainloader):
                if self.is_cuda:
                    x = x.cuda()
                ...
                pbar.set_description(f'loss = {loss.item()}, log(p(x)) = {z.mean().item()}')  # ! ERROR?
